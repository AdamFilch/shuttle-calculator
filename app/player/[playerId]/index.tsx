...
import { payCourtBySessionId, paySessionInFull, payShuttleInstancesByIds } from "@/services/shuttle-payments";
...
export default function SelectPlayerPage() {
    ...
    const toggleCharge = (charge: ShuttleInstanceCharge) => {
        setSelectedCharges((prev) =>
            prev.some((v) => v.shuttle_instance_id === charge.shuttle_instance_id)
                ? prev.filter((v) => v.shuttle_instance_id !== charge.shuttle_instance_id)
                : [...prev, charge]
        )
    }
    ...
    const handleConfirm = async () => {
        ...
        if (pendingAction.type === 'all') {
            await paySessionInFull({ sessionId: pendingAction.sessionId, player_id: playerId.toString() })
        } else if (pendingAction.type === 'court') {
            await payCourtBySessionId({ sessionId: pendingAction.sessionId, player_id: playerId.toString() })
        } else {
            await payShuttleInstancesByIds({ shuttleCharges: selectedCharges, player_id: playerId.toString() })
            setSelectedCharges([])
        ...
        <SafeAreaView className="flex-1 bg-background-50">
            ...
            <ScrollView className="flex-1 px-4">
                <VStack space="md" className="pb-8 pt-2">
                        ...
                        <HStack space="sm">
                            {selectShuttleMode ? (
                                <>
                                    ...
                                    >
                                        <ButtonText>Pay Selected ({selectedCharges.length})</ButtonText>
                                    </Button>
                                    <Button
                                        variant="outline"
                                        action="secondary"
                    ...
                    {sessionsOwed.length === 0 ? (
                        ...
                        sessionsOwed.map((session) => {
                            const sessionTotal = session.shuttle_total_owed + session.court_total_owed
                            const isExpanded = expandedSessions.has(session.session_id)

                            return (
                                <VStack key={session.session_id} space="sm">
                                    <ListRow
                                        title={session.name === '' ? (DisplayTimeDDDASHMMDASHYYYY(session.date) ?? '') : session.name}
                                        subtitle={`Total owed: $${sessionTotal.toFixed(2)}`}
                                        trailing={
                                            <Icon as={isExpanded ? ChevronDownIcon : ChevronRightIcon} className="text-typography-500" />
                                        }
                                        ...
                                        <VStack space="sm" className="pl-3">
                                            <HStack space="sm">
                                                <Button
                                                    size="sm"
                                                    onPress={() => {
                                                        setPendingAction({ type: 'all', sessionId: session.session_id, amount: sessionTotal })
                                                    }}
                                                >
                                                    <ButtonText>Pay All (${sessionTotal.toFixed(2)})</ButtonText>
                                                </Button>
                                                {session.court_total_owed > 0 && (
                                                    <Button
                                                        ...
                                                        action="secondary"
                                                        onPress={() => {
                                                            setPendingAction({ type: 'court', sessionId: session.session_id, amount: session.court_total_owed })
                                                        }}
                                                    >
                                                        <ButtonText>Pay Court Only (${session.court_total_owed.toFixed(2)})</ButtonText>
                                                    </Button>
                                                )}
                                            </HStack>
                                            ...
                                            {session.matches.length === 0 ? (
                                                ...
                                                session.matches.map((match) => {
                                                    const matchOwed = match.charges.reduce((acc, c) => acc + c.owed_amount, 0)
                                                    const isMatchExpanded = expandedMatches.has(match.match_id)
                                                    ...
                                                    return (
                                                        <VStack key={match.match_id} space="xs">
                                                            <ListRow
                                                                ...
                                                                trailing={
                                                                    <Icon as={isMatchExpanded ? ChevronDownIcon : ChevronRightIcon} className="text-typography-400" />
                                                                }
                                                            ...
                                                            {isMatchExpanded && (
                                                                <VStack space="xs" className="pl-3">
                                                                    <Text size="xs" className="text-typography-500">
                                                                        Players: {match.players.length > 0 ? match.players.map((p) => p.name).join(", ") : "None recorded"}
                                                                    </Text>
                                                                    {match.charges.map((charge) => {
                                                                        ...
                                                                        return (
                                                                            <ListRow
                                                                                ...
                                                                                title={charge.name}
                                                                                subtitle={isPaid ? "Paid" : `$${charge.owed_amount.toFixed(2)}`}
                                                                                selected={selectShuttleMode && isSelected}
                                                                                trailing={
                                                                                    selectShuttleMode && !isPaid ? (
                                                                                        <Checkbox
                                                                                            value={String(charge.shuttle_instance_id)}
                                                                                ...
                                                                                onPress={selectShuttleMode && !isPaid ? () => toggleCharge(charge) : undefined}
                                                                            />
                                                                        )
                                                                    })}
                                                                </VStack>
                ...
                <PaymentConfirmationDialog
                    ...
                    title={
                        pendingAction?.type === 'all' ? "Pay Session in Full" :
                            pendingAction?.type === 'court' ? "Pay Court Only" :
                                "Pay Selected Shuttles"
                    }
                    description={
                        pendingAction?.type === 'all' ? "This pays off every remaining shuttle and court charge for this session. This action is irreversible." :
                            pendingAction?.type === 'court' ? "This pays only the court cost for this session -- shuttle charges are left unpaid. This action is irreversible." :
