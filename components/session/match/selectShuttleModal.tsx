...
export const SelectShuttleModal = React.memo(function SelectShuttleModal({
    ...
    const [instanceList, setInstanceList] = useState<ShuttleInstance[]>([])
    const [currentSelectedShuttle, setCurrentSelectedShuttle] = useState<number | null>(null)
    const [currentSelectedInstance, setCurrentSelectedInstance] = useState<number | null>(null)
    ...
    const alreadyReusedIds = new Set(
        selectedShuttles
            .filter((s): s is Extract<ShuttleSelection, { mode: 'reused' }> => s.mode === 'reused')
    ...
    useEffect(() => {
        ...
        fetchAllShuttles().then((res) => {
            setShuttleList(res)
            if (res.length > 0) {
                setCurrentSelectedShuttle(res[0].shuttle_id)
            }
        });
        fetchShuttleInstancesBySessionId(sessionId).then((res) => {
            const selectable = res.filter((instance) => !alreadyReusedIds.has(instance.shuttle_instance_id))
    ...
    const canConfirm =
        mode === 'new' ? (currentSelectedShuttle !== null && parseInt(numberShuttles) > 0) :
        mode === 'reused' ? currentSelectedInstance !== null :
        true
                ...
                <VStack space="md">
                    ...
                    {mode === 'new' && (
                        shuttleList && shuttleList.length > 0 ? (
                            <VStack space="md">
                                <Select
                                    selectedValue={currentSelectedShuttle !== null ? String(currentSelectedShuttle) : undefined}
                    ...
                    {mode === 'reused' && (
                        instanceList.length > 0 ? (
                            <Select
                                selectedValue={currentSelectedInstance !== null ? String(currentSelectedInstance) : undefined}
                                onValueChange={(val) => setCurrentSelectedInstance(parseInt(val))}
                            >
                                <SelectTrigger variant="outline" size="lg">
                                    <SelectInput className="flex-1" placeholder="Select a shuttle in play" />
                                    <SelectIcon className="mr-3" as={ChevronDownIcon} />
                                </SelectTrigger>
                                <SelectPortal>
                                    <SelectBackdrop />
                            ...
                            <Text size="sm" className="text-typography-500">
                                {alreadyReusedIds.size > 0
                                    ? "No more shuttles left to reuse — every one introduced this session is already selected."
                                    : "No shuttles have been introduced this session yet."}
                            </Text>
                        )
                    )}
                    ...
                    {mode === 'free' && (
                        <Text size="sm" className="text-typography-500">
                            Attaches a free shuttle to this match — contributes $0 to the split.
                        </Text>
                    )}
                </VStack>
            </ModalBody>
            <ModalFooter>
                ...
                <Button
                    ...
                    onPress={() => {
                        ...
                        } else if (mode === 'reused' && currentSelectedInstance !== null) {
                            onSelect({
                                mode: 'reused',
